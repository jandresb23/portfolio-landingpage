...
- La aplicación arranca sin errores de sintaxis o importación con
  `uvicorn app.main:app`.
- Con `SYNC_SECRET_KEY` sin definir, `/sync` responde `503` y el resto de
  endpoints funciona con normalidad.
- Con `SYNC_SECRET_KEY` definido y un token correcto, `/sync` puebla
  `github_cache` y `youtube_cache` sin duplicados al ejecutarse dos veces
  seguidas.
- El frontend muestra un mensaje de error visible si el backend no responde.
